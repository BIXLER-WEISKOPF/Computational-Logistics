...
def putData(cityList, bestPath, totalDistance, filepath, distMatrix):
    #getting the workbook ready
    wb = pyxl.load_workbook(filepath)
    ws = wb['Sheet1']
    
    #displaying the distance matrix
    ws.cell(row = 4, column = 5).value = str("Distance Matrix")
    
    #city labels for the left side
    for i in range(0,len(cityList)):
        ws.cell(row = 6 + i, column = 5).value = cityList[i][0]
    
    #city labels for the top side
    for i in range(0,len(cityList)):
        ws.cell(row = 5, column = 6 + i).value = cityList[i][0]
    
    #populating the distance matrix
    for row in range(0,len(cityList)):
        for col in range(0,len(cityList)):
            ws.cell(row = row + 6, column = col + 6).value = distMatrix[row,col]
            
    #displaying optimal path
    ws.cell(row = 1, column = 5).value = str("Travel Path:")
    for i in range(0, len(bestPath)):
        ws.cell(row = 1, column = 6 + i).value = travelPath[i]
    
    #displaying total cost
    ws.cell(row = 2, column = 5).value = str("Total Distance:")
...
putData(cityList, travelPath, totalDistance, file, distMat)
